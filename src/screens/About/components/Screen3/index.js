...
const Screen3 = () => {
    ...
    useEffect(() => {
        ratingLocation?.map(i => {
            const miles = calculateDistance(userLocation?.lat, userLocation?.long, i?.location?.lat, i?.location?.long)
            i.miles = Math.floor(miles);
        }
        )
        let filterdMiles = ratingLocation?.filter(x => x?.miles < selectedValues?.miles)
    ...
    useEffect(() => {
        ...
        let filterdMiles = ratingLocation?.filter(x => x?.miles < selectedValues?.miles || value)
        setFilterMilesData(filterdMiles)
    }, [isFocused])
    ...
    const submit = () => {
        console.log("TCL ~ file: index.js ~ line 69 ~ submit ~ fiterMilesData", fiterMilesData)
        if (toggle && fiterMilesData) {
            dispatch({ type: 'selectedValues', payload: { ...selectedValues, miles: value, fiterMilesData } })
            alert('miles added')
        }
    }
        ...
        <>
            ...
            <View style={toggle ? styles.screen3__center : styles.screen3__disable}>
                ...
                <View style={{ marginTop: 20, width: '100%', alignItems: 'center' }}>
                    <Button onClick={() => submit()} disable={!toggle && disable} customStyle={{ marginTop: 10 }} text="Done" />
                </View>
            </View>
        </>
    )
